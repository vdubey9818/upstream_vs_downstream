...
f_down = np.full((row_index, len(N_values)), np.nan)
...
for j, N in enumerate(N_values):
    ...
    f_down[index_vals, j] = data[:, 2]
    f_def[index_vals, j] = data[:, 3]
    C[index_vals, j] = data[:, 4]
...
plots = [
    (f_up, "Upstream", hex_cmap("#FF0000", "coral")),
    (f_down, "Downstream", hex_cmap("#4F7942", "green")),
    (f_def, "Defectors", hex_cmap("#6082B6", "blue")),
    (C, "Cooperation", hex_cmap("#555555", "grey"))
]

#ploting
for ax, (matrix, title, cmap) in zip(axs.flat, plots):
    im = ax.imshow(matrix,
                   origin='lower',
                   aspect='auto',
                   cmap=cmap,
    ...
    cbar.ax.tick_params(labelsize=10)  # increase tick fontsize

plt.tight_layout()
plt.savefig('number_vs_degree.png',dpi=300)
...
zoom_down = f_down[row_start:row_end+1, col_start:col_end+1]

fig, ax = plt.subplots(figsize=(4,3))

im = ax.imshow(zoom_down,
               origin='lower',
               aspect='auto',
               cmap=hex_cmap("#4F7942", "green"),
               interpolation='none',vmin=0.0, vmax=1.0)
...
plt.tight_layout()
plt.savefig('zoomed_down.png',dpi=300)
