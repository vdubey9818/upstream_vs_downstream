...
@njit
def fermi_update(kappa, payoff, beta, mutation_rate, ownimage):
    N = len(kappa)
    fraction=0.1
    ...
    for _ in range(num_updates):
        ...
        if np.random.random() < prob:
            if np.random.random() > mutation_rate:
                ...
                kappa[i] = np.random.choice(np.array([0,2]))
            ownimage[i] = 1
...
for gen in tqdm(range(num_generations), desc="Simulating"):
    ...
    fermi_update(kappa, payoff,beta,mutation_rate, ownimage)
