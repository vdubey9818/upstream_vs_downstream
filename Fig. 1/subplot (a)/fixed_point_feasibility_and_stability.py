...
# =================================
data = np.genfromtxt("jacobian_data.txt", missing_values="NA", filling_values=np.nan)
...
# common denominator
D = c*(-gd + gu)*h + b*(-gu**2 + gd*h)

# x1*
x_term1 = safe_div((b - c)*h*gd, b*gu**2)

x_num2 = (
    c**2*(2*gd - gu)*h +
    b**2*(-gu**2 + gd*h) +
    b*c*(gu**2 - 3*gd*h + 2*gu*h)
)
x_term2 = safe_div(x_num2, c**2*h*D)

xfixed = x_term1 + mu * x_term2

# x2*
y_term1 = safe_div(c*h, b*gu)

y_num2 = gu * (c*(2*gd - gu)*h + b*(gu**2 - 2*gd*h))
y_term2 = safe_div(y_num2, (b - c)*gd*h*D)

yfixed = y_term1 + mu * y_term2
...
for i in range(len(J11)):
    if np.any(np.isnan([J11[i], J12[i], J21[i], J22[i]])):
        ...
        eigvals.append(np.sort(vals)) # arrange in ascending order
...
np.save("region_fp1.npy", final_region) #save this to find the bistable region
plt.show()